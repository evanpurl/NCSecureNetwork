...
def userembed(bot, user, server):
    embed = discord.Embed(title="**Welcome!**", description=f"Welcome to the {server.name} {user.mention}", color=discord.Color.dark_red(),
...
class memberfunctions(commands.Cog):
    ...
    async def on_member_join(self, member):
        try:
            wchannel = await dbget(member.guild.id, "ncsn", "welcomechannelid")
            ...
            roleid = await dbget(member.guild.id, "ncsn", "defaultroleid")
    ...
    async def on_member_remove(self, member):
        ...
        channel = discord.utils.get(member.guild.channels, id=wchannel[0])
        if channel:
            await channel.send(f"{member.name} has left the {member.guild.name}.")
    ...
    async def welcomechannel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        try:
            await dbset(interaction.guild.id, "ncsn", "welcomechannelid", channel.id)
    ...
    async def resetwelcomechannel(self, interaction: discord.Interaction):
        try:
            await dbset(interaction.guild.id, "ncsn", "welcomechannelid", 0)
